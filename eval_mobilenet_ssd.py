...
def main(_):
    ...
    output_dir='/home/sixd-ailabs/Downloads/VOCdevkit/test'
    source_img_dir='/home/sixd-ailabs/Downloads/VOCdevkit/VOC2007/JPEGImages'
    ...
    for class_ind in range(1, FLAGS.num_classes):
        with open(os.path.join(summary_dir, 'results_{}.txt'.format(class_ind)), 'wt') as f:
            for image_ind, pred in enumerate(det_results):
                ...
                name_str=filename.decode('utf8')

                shape = pred['shape']
                if result_dict.get(name_str) is None:
                    result_dict[name_str]=[shape]
                ...
                for det_ind in range(valid_mask.shape[0]):
                    ...
                    result_dict[name_str].append((
                        bboxes[det_ind, 1], bboxes[det_ind, 0],
                        bboxes[det_ind, 3], bboxes[det_ind, 2],
                        class_ind, scores[det_ind]))


    for filename in result_dict:

        items=result_dict[filename]
        shape=items[0]
        writer = PascalVocWriter('test', filename, shape, localImgPath='')
        for item in items[1:]:
            writer.addBndBox(item[0],item[1],item[2],item[3],CLASSES[item[4]],False,item[5])
        writer.save(os.path.join(output_dir,filename[:-4]+'.xml'))
